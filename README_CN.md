...
| 📅 日历视图 | 月度日历、颜色标记、详情侧栏、今日任务状态、连续打卡天数 |
...
- **任务完成分组按完成日期显示**：单次任务和量化任务达到总目标后，完成当天进入「今日已完成」，之后永久归入「过去完成」，任务条显示「完成于 yyyy-MM-dd」；回看完成前日期时仍显示为进行中
- **今日目标与日历统计按日期过滤**：今日目标只列出当天出现的任务，未设置每日目标的量化任务不参与该统计；日历统计同步采用当天任务口径
- **日历统计调整**：「打卡率」改为所选任务的「今日任务：已完成/未完成」；连续打卡改为当天至少完成一个任务即算成功，无任务日不打断
- **量化每日目标改为增量口径**：累加模式按“今日值 - 今日首次访问前基线 ≥ 每日目标”判定，更新模式保留绝对值口径；跨日自动记录基线，避免把昨天的累计量误算成今天完成
- **云同步升级为三端并行**：PC 会同时同步所有已配置的 GitHub、Gitee、WebDAV；反馈逐一列出每个平台的上传/下载/合并/冲突和失败原因
- **计时、专注、打卡、健康、喝水记录支持条目级合并**：新增 Uid，全局唯一记录在电脑和手机同时产生时自动合并，不再整文件覆盖丢失；旧记录会在合并时自动补 Uid
